...
public class ValidateCodeUtil {
    ...
    public static Map<String, String> getRandomCodeImage() {
        // 随机字符
        String randomStr = generateVerifyCode(4);
        log.info("随机字符：" + randomStr);
        try {
            ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
            //  将图片以png格式返回,返回的是图片
            outputImage(165, 45, byteArrayOutputStream, randomStr);
            byte[] bytes = byteArrayOutputStream.toByteArray();
            Map<String, String> result = new HashMap<>();
            result.put("key", randomStr);

            result.put("base64", encoder.encodeToString(bytes).trim().replaceAll("[\\s*\t\n\r]", ""));
            return result;
        } catch (Exception e) {
            e.printStackTrace();
        }
